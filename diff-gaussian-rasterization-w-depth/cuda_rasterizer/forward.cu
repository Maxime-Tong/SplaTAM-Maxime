	...
	bool prefiltered)
{
	...
	float3 p_view;
	if (!in_frustum(idx, orig_points, viewmatrix, projmatrix, prefiltered, p_view)) // 19 MACs
		return;

	// Transform point by projecting
	float3 p_orig = { orig_points[3 * idx], orig_points[3 * idx + 1], orig_points[3 * idx + 2] };
	float4 p_hom = transformPoint4x4(p_orig, projmatrix); // 12 MACs
	float p_w = 1.0f / (p_hom.w + 0.0000001f); // 1MACs
	...
	{
		computeCov3D(scales[idx], scale_modifier, rotations[idx], cov3Ds + idx * 6); // 27+1=28 MACs
		cov3D = cov3Ds + idx * 6;
	}

	// Compute 2D screen-space covariance matrix
	float3 cov = computeCov2D(p_orig, focal_x, focal_y, tan_fovx, tan_fovy, cov3D, viewmatrix); // 63 MACs
	...
	float lambda1 = mid + sqrt(max(0.1f, mid * mid - det)); // 1 MACs
	float lambda2 = mid - sqrt(max(0.1f, mid * mid - det)); // 1 MACs
	float my_radius = ceil(3.f * sqrt(max(lambda1, lambda2)));
	float2 point_image = { ndc2Pix(p_proj.x, W), ndc2Pix(p_proj.y, H) }; // 4 MACs
	uint2 rect_min, rect_max;
	getRect(point_image, my_radius, rect_min, rect_max, grid); // 4 MACs
	...
	{
		glm::vec3 result = computeColorFromSH(idx, D, M, (glm::vec3*)orig_points, *cam_pos, shs, clamped); // 17 MACs
	...
	tiles_touched[idx] = (rect_max.y - rect_min.y) * (rect_max.x - rect_min.x); // 1 MACs
}

// # of MACs per pixel = 3 + r * (1 + 256 * 14) + 6
...
// 	float D = 0.0f;  // Mean Depth
	...
	// # of MACs = rounds * (1 + BLOCK_SIZE * 14)
